...
async function checkFinished(jobId, statusUrl, authOptions, finisher) {
    ...
    let response = await got(statusUrl, getOptions);
    ...
    let jobResult;
    try {
        jobResult = JSON.parse(response.body);
    } catch (error) {
        throw new Error(`Failed to parse job result: ${error}\nURL was: ${statusUrl}\nJob result: ${util.inspect(jobResult)}`);
    }
    ...
    static async runApply({ cds }, options) {
        ...
        .then(response => {
            ...
            return new Promise((resolve, reject) => {
                const statusUrl = Apply.concatUrls(options.appUrl, statusPath);
                const finisher = {
                    resolve,
                    reject,
                    finished: false
