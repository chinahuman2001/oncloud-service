...
class SettingsManager {
    ...
    static async loadAndMergeSettings(options, logout = false) {
        ...
        logger.debug(`Effective project settings: ${format(params)}`);
    ...
    static async addAuth(params, logout) {
        const { askQuestion } = require('./helper/question');
        ...
        async function addClientSecret() {
            params.clientsecret = await askQuestion('clientsecret: ', undefined, true);
            if (params.clientsecret === '') {
                throw new Error('clientsecret cannot be empty');
            }
        }
        async function addPasscode() {
            let prompt = `Passcode${params.passcodeUrl ? ' (visit ' + params.passcodeUrl + ' to generate)' : ''}: `;
            params.passcode = await askQuestion(prompt, undefined, true);
        ...
        function relevant(auth) {
            let relevantAuth = {};
            if (auth.keyring.tokenStorage || auth.plain.tokenStorage) {
                // Select storage and collect data.
                // Note: If `keytar` is not functional, keyring storage will not be selected.
                if (auth.keyring.tokenStorage && auth.plain.tokenStorage) {
                    ...
                    relevantAuth = Object.assign(auth[storage], {
                    ...
                    relevantAuth = Object.assign(auth[other(params.tokenStorage)], {
        ...
        if (params.username) {
            if (!params.password && !logout) {
                await addPassword();
            }
            return;
        }

        if (params.clientid && !params.clientsecret && !logout) {
        ...
        Object.assign(params, relevant(auth));

        if (!params.token && !params.passcode && !logout) {
            await addPasscode();
        }
    }
    ...
    static async _saveAuthToFile(settings, auth) {
        ...
        if (auth) {
            ...
            notifyLoggedIn(settings)(`Saved authentication data to ${location}`);
