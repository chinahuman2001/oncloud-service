...
const { isAspect } = require('../model/modelUtils');
...
function resolve( model ) {
  ...
  let newAutoExposed = [];
  const enableExpandElements = isBetaEnabled( options, 'subElemRedirections' );
  return main();
  ...
  function main() {
    ...
    // TODO v2: run silent cycle detection here and stop all expansions (via
    // enableExpandElements = false) after that if there are cycles.  New
    ...
    // for builtin types
    forEachGeneric( model.definitions.cds, 'artifacts', chooseAnnotationsInArtifact);
    forEachGeneric( model.definitions['cds.hana'], 'artifacts', chooseAnnotationsInArtifact);
    ...
    // v2: But if we have run a silent cycle detection earlier, we can use the
  ...
  // TODO: in v2, the name-resolution relevant properties
  // (elements,items,target) are proxy-copied anyway, i.e. the finalType of an
  // artifact will always be the artifact itself.  We might need some
  // similar property (_originRoot ?) to detect cyclic element expansions, though
  ...
  function expandElements( art, struct ) {
    ...
    const location = ref && ref.location || art.name && art.name.location || art.location;
  ...
  function redirectImplicitly( elem, assoc ) {
    ...
    if (service && target._service !== service) {
      ...
      const exposed = minimalExposure( target, service );
  ...
  function minimalExposure( target, service ) {
    const descendants = target._descendants && target._descendants[service.name.absolute] || [];
  ...
  function dependentAutoExposedName( target, service ) {
    ...
    const exposed = minimalExposure( base, service );
    if (exposed.length === 1)   // unique explicit projection of base
      return exposed[0].name.absolute + target.name.absolute.slice( base.name.absolute.length );
    // now use the name part of the bases
    // TODO: do this generally for sub artifacts of non-contexts, but keep
    // _base as long as we have deprecated.
    let parent = base;
    while (parent._base)
      parent = parent._base;
    return (parent._parent)
      ? service.name.absolute +
        target.name.absolute.slice( parent._parent.name.absolute.length )
      : `${ service.name.absolute }.${ target.name.absolute }`;
  }
  ...
  function resolveRefs( art ) {
    ...
    let obj = art.returns || art;
    if (obj.type)             // TODO: && !obj.type.$inferred ?
      resolveTypeExpr( obj, art );
    const type = finalType( obj ); // make sure implicitly redirected target exists
    if (!obj.items && type && type.items &&
        isBetaEnabled( options, 'subElemRedirections' )) {
      const items = {
        origin: {},
      ...
      setProp( items.origin, '_artifact', type.items );
      obj.items = items;
    }
    ...
    if (obj.type) {             // TODO: && !obj.type.$inferred ?
      ...
      // TODO: in the future, the reject functions need to get some info about the caller
      // this check would move there to allow it being used in code completion
      if (art.kind === 'element' && art._main && art._main.kind === 'type' &&
          !isAspect(art._main) && obj.type._artifact && obj.targetAspect) {
        const isUnnamedAspect = !!obj.targetAspect.elements;
        const isNamedAspect = isAspect(obj.targetAspect._artifact);

        if (isUnnamedAspect || isNamedAspect) {
          message( 'type-managed-composition', obj.targetAspect.location, art, {}, 'Warning',
                   'Managed compositions cannot be used in types' );
        }
      }
    }
    if (obj.target) {
  ...
  // elements, we create a localized variant for it), and finally make sure via
  // redirection that associations in localized convenience views have as target
  ...
  function resolveExpr( expr, expected, user, extDict) {
    ...
    if (expr.path) {
      ...
      for (const step of expr.path) {
        if (step && (step.namedArgs || step.where || step.cardinality) &&
