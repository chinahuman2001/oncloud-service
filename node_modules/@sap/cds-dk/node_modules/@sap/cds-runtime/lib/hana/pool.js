      ...
      new Promise((resolve, reject) => {
        credentials4(tenant, credentials)
          .then(creds => {
             ...
             * If it fails due to invalid credentials:
             *   { code: 10, level:1, message:'authentication failed', position:0, sqlState:'28000' }
             * we delete the current pool from the pools map and overwrite the pool factory create function.
            ...
            p.on('factoryCreateError', async function (err) {
              if (err.code === 10) {
                LOG._warn &&
                  LOG.warn(
                    `Could not establish connection for tenant "${tenant}" due to invalid credentials. Existing pool will be drained.`
                  )
                pools.delete(tenant)
