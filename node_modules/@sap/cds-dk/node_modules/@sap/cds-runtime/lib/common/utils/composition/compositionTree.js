...
const _diffData = (newData, oldData, entity, newEntry, oldEntry, definitions) => {
  ...
  for (const key of keysSet.keys()) {
    ...
    if (fk && newVal === undefined && oldVal !== undefined) {
      ...
      if (nav.isComposition && nav.is2one && !definitions[nav.target]._hasPersistenceSkip) {
        result[key] = null
      }
    }
  }
...
const hasCompositionDelete = (definitions, cqn) => {
  if (cqn && cqn.DELETE && cqn.DELETE.from) {
    ...
    if (entity) {
      return !!Object.keys(entity.elements || {}).find(k => entity.elements[k]._isCompositionEffective)
    }
  }
  return false
}
...
const _addSubCascadeDeleteCQN = (compositionTree, level, cqns, draft, elementSet = new Set()) => {
  for (const element of compositionTree.compositionElements) {
    ...
    if (allBackLinks.length > 0 || element.links.length > 0) {
      ...
      _addSubCascadeDeleteCQN(element, level + 1, cqns, draft, elementSet)
    }
  }
