...
const { isBoundAction, getBoundActionBindingParameter } = require('../../cds-services/services/utils/draftUtils')
...
const _processCategory = ({ row, key, category, isRoot, event, value }) => {
...
const processorFn = (errors, req) => {
  ...
  return (row, key, element, plain, isRoot, pathSegments, suppressErrorPropagation = false) => {
    ...
    for (const category of categories) {
      _processCategory({ row, key, category, isRoot, event, value })
    }
    ...
    checkInputConstraints({ element, value: value.val, errors, pathSegments })
  }
}
...
function _handler(req) {
  ...
  for (const row of arrayData) {
    ...
    if (isBoundAction(req)) {
      const pathSegment = getBoundActionBindingParameter(req)
      const keys = req.context.params[0]
      pathSegments = pathSegment ? [pathSegment] : []

      if (Object.prototype.hasOwnProperty.call(keys, 'IsActiveEntity')) {
...
const _processActionFunctionRow = (row, param, key, errors, event, service) => {
  ...
  if (value != null && param._type && KINDS_TO_VALIDATE[param._type.kind]) {
    ...
    for (const row of valArray) {
      const args = {
        processFn: processorFnForActionsFunctions(errors, key),
        row,
        template
      }
      templateProcessor(args)
