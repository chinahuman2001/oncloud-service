...
const virtualHandler = require('../../../db/generic/virtual')
...
function _copyCQNPartial(partial) {
  if (partial.SELECT && partial.SELECT.where) {
    ...
    newPartial.SELECT.columns = _copyArray(partial.SELECT.columns)
...
const _validatedActiveWithDraftInProcess = (req, draftWhere, draftParameters, columns) => {
  if (draftParameters.draftInProcessByUser.op === '!=') {
    return _isValidWithDraftLocked(
      ...
      ? _activeWithDraftInProcess(req, draftWhere, columns, req.user.id)
      : undefined
  }
  ...
  return _isValidWithDraftTimeout(
    ...
    ? _activeWithDraftInProcess(req, draftWhere, columns, null)
    : undefined
}

const _validatedExcludeActiveDraftExists = (req, draftWhere, draftParameters, columns) =>
  _isValidExcludeActiveDraftExists(draftParameters.isActiveEntity, draftParameters.siblingIsActive)
    ? _excludeActiveDraftExists(req, draftWhere, columns)
    : undefined

const _validatedActive = (req, draftWhere, draftParameters, columns) =>
  _isValidActive(draftParameters.isActiveEntity, req, draftWhere) ? _active(req, draftWhere, columns) : undefined
...
const _validatedDraftOfWhichIAmOwner = (req, draftWhere, draftParameters, columns) =>
  _isValidDraftOfWhichIAmOwner(draftParameters.isActiveEntity)
    ? _draftOfWhichIAmOwner(req, draftWhere, columns)
    : undefined
...
const _generateCQN = (reqOriginal, req, columns, serviceName, model) => {
  ...
  if (draftParameters.siblingIsActive) {
    return draftParameters.draftInProcessByUser
      ? _validatedActiveWithDraftInProcess(req, req.query.SELECT.where, draftParameters, columns)
      : _validatedExcludeActiveDraftExists(req, req.query.SELECT.where, draftParameters, columns)
  }
...
const _handler = async function (req) {
  ...
  const sqlQuery = cqn2cqn4sql(req.query, req[Symbol.for('sap.cds.model')])
  ...
  // REVISIT: We remove selected virtual columns already here, because we do not want to pollute the handler with draft stuff
  // violates separation of concerns, but is cleaner as we do not need to cover all possible joins/unions in handler
  virtualHandler.call(this, reqClone)
  ...
  if (cqnScenario.scenario === 'DRAFT_ADMIN') {
    ...
    _calculateDraftAdminColumns(result[0], req.user.id)
  }

  calculateDraftTimeout(cqnScenario.scenario, result)

  if (cqnScenario.scenario === 'SIBLING_ENTITY') {
    if (result.length === 0 || result[0].draftAdmin_inProcessByUser !== req.user.id) {
      return []
    }
