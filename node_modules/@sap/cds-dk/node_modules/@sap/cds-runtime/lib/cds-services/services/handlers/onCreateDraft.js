...
const _getUpdateDraftAdminCQN = ({ user }, draftUUID) => {
...
const _getInsertDraftAdminCQN = ({ user }, uuid) => {
...
const _getInsertDataCQN = (context, draftUUID) => {
  ...
  const insertData = INSERT.into(draftName).entries(context.query.INSERT.entries[0]) // entries is always set because there are no entities without keys
...
const _handler = (req, next) => {
  ...
  return Promise.all([dbtx.run(adminDataCQN), dbtx.run(insertDataCQN)])
    .then(() => {
      return dbtx.run(readInsertDataCQN)
    })
    .then(result => {
      if (result.length === 0) {
        req.reject(404)
