...
const isBoundAction = req => {
  const context = req.context
  return !!(context && context.getUriInfo && context.getUriInfo().getLastSegment().getKind() === 'BOUND.ACTION')
}

const getBoundActionBindingParameter = req => {
  const bindingParameterName = req.target.actions[req.context.event]['@cds.odata.bindingparameter.name']

  // 'in' is the default binding parameter name for bound actions/functions
  return bindingParameterName === undefined ? 'in' : bindingParameterName
}
...
const hasKeyInWhere = (where, target) => {
  ...
  const key = getKeyProperty(target)
...
module.exports = {
  ...
  isBoundAction,
  getBoundActionBindingParameter,
  ensureDraftsSuffix,
