...
const cli = { //NOSONAR
  ...
  exec (cmd = process.argv[2], ...argv) {
    ...
    const task = this.load (cmd, /*else*/ ()=> argv.unshift(cmd) && _defaultCompile (argv))
    if (task) {
  ...
  load (cmd,_default) {
    return /^(\.[/\\])?cds/i.test(cmd) && _dieForUnknownCommand (cmd)  // cds cds
...
function _prepareCommands() {
  // install cds.compile.to.openapi exporter for usage during runtime
  const cds = require(_local('@sap/cds'))
  Object.defineProperty(cds.compile.to, 'openapi', { configurable:true, get: ()=>require('../lib/compile/openapi') })
}
