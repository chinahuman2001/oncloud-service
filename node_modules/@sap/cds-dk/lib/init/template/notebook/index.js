...
module.exports = class NotebookTemplate extends TemplateBase {
    ...
    async finalize() {
        if (this.hasVscode && this.hasNotebookExtension) {
            this.logOutput = `\nTo view the notebook in VS Code:
        1. ${term.info('Ctrl + Click')} the following link: ${term.info(`${this.nb_json}`)}
        2. Make sure to ${term.info('Trust')} the notebook in the popup window to enable markdown and code execution
        3. Run the notebook\n`;
        }
        if (this.pythonPackages.includes('notebook') && this.hasJupyterlab) {
            this.logOutput += `\nTo view the notebook in JupyterLab:
        1. Open with JupyterLab: ${term.info(`python -m jupyterlab ${path.relative(process.cwd(), this.nb_json)}`)}
        2. Run the notebook\n`;
        }
        this.logOutput += `\nTo view the notebook in your browser:
        1. Open with Jupyter: ${term.info(path.join(this.notebookEnv, this.binPythonVenv, 'jupyter') +
            ' notebook ' + path.relative(process.cwd(), this.nb_json))}
        2. Run the notebook\n`;
        this.logger.log(this.logOutput);
    }
}
