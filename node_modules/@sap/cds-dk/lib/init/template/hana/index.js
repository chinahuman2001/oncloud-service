...
module.exports = class HanaTemplate extends TemplateBase {
    ...
    async _updatePomXml() {
        ...
        if (!this.options.add.has(OPTION_JAVA)) {
            ...
            try {
                ...
                }, (process.env.DEBUG ? this.logger : nullLogger));
            } catch (err) {
    ...
    async _updateCdsrcJson(cdsrcJsonPath, modelName) {
        ...
        if (process.env.DEBUG) {
            this.logger.debug(`_updateCdsrcJson before: ${JSON.stringify(cdsrcJson, null, 2)}`);
        }
        ...
        if (!this._isUsingHana(modelName)) {
            ...
            if (process.env.DEBUG) {
                this.logger.debug(`_updateCdsrcJson written: ${JSON.stringify(cdsrcJson, null, 2)}`);
            }
        }
    }
